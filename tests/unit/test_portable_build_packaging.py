...
def test_drop_target_spec_builds_console_one_folder_drop_exe() -> None:
    ...
    assert "output" not in spec_text
...
def test_build_portable_script_runs_tests_builds_zip_and_copies_handoff_files() -> None:
    ...
    assert r"app\config\defaults.json" in script_text
    assert "rules" in script_text
    assert "templates" in script_text
    assert "tools" in script_text
    assert "$OutputDir" in script_text
    assert "_internal\\TDSYNNEX-CB-LogParser.exe" in script_text
    assert "Portable package is missing required path" in script_text
...
def test_portable_readme_documents_desktop_case_output() -> None:
    readme_path = REPO_ROOT / "packaging" / "portable" / "README.txt"
    readme_text = readme_path.read_text(encoding="utf-8")

    assert readme_path.exists()
    assert "Desktop\\TDSYNNEX-CB-LogParser" in readme_text
    assert "Case-<CASE_NUMBER>-scan-YYYYMMDD-HHMMSS" in readme_text
    assert "--output" in readme_text
    assert "bundled output folder" not in readme_text
    assert "Documents" not in readme_text
    assert "--open-output and --pause" in readme_text
