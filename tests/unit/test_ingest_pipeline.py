...
def test_normal_zip_extraction_writes_artifact_coverage(tmp_path: Path) -> None:
    ...
    assert any(row["artifact_path"].endswith("logs/app.log") for row in rows)
...
def test_nested_zip_extraction(tmp_path: Path) -> None:
    ...
    assert any(row["artifact_path"].endswith("archives/inner.zip") for row in rows)
    assert any(row["artifact_path"].endswith("nested/result.json") for row in rows)
...
def test_zip_slip_path_traversal_is_refused(tmp_path: Path) -> None:
    ...
    assert any(
        row["artifact_path"].endswith("unsafe.zip!../evil.txt")
        and row["decoder_status"] == "unsafe-archive-path-refused"
        and row["review_mode"] == "skipped-by-policy"
        for row in rows
    )
    assert any(row["artifact_path"].endswith("safe/good.log") for row in rows)


def test_unsupported_artifact_is_recorded_as_specialized_tool_unavailable(tmp_path: Path) -> None:
