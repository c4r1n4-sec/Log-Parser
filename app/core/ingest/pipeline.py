...
from app.core.util.config import ExtractionLimits, ScanSettings
...
class IngestPipeline:
        ...
        for input_path in inputs:
            ...
            if resolved.is_dir():
                for file_path in self._iter_files(resolved):
                    self._process_file_safely(
                        file_path, original_input=resolved, extracted_from=None, depth=0
                    )
            else:
                self._process_file_safely(
                    resolved, original_input=resolved, extracted_from=None, depth=0
                )
        return self.rows
    ...
    def _process_file_safely(
        ...
        try:
            self._process_file(path, original_input, extracted_from, depth)
        except Exception as exc:  # keep one bad artifact from preventing reports
            LOGGER.exception("Artifact processing failed: %s", path)
            self._record_policy_row(
                artifact_path=path,
                original_input_path=original_input,
                extracted_from=extracted_from,
                artifact_family="unknown",
                identified_by="processing:error",
                decoder_status="scan-failed",
                limitations=f"Artifact processing failed: {exc}",
    ...
    def _scan_artifact(self, path: Path, classification: ArtifactClassification) -> ScanStats:
        try:
            if should_scan_text_family(classification.artifact_family):
                return scan_text_file(path, self.scan_settings)
            if should_scan_binary_family(classification.artifact_family):
                return scan_binary_file(path, self.scan_settings)
    ...
    def _coverage_row(
        ...
        return ArtifactCoverageRow(
            ...
            root_cause_value="",
            related_rule_ids="",
            limitations=self._row_limitations(path, extracted_from, scan_stats.limitations),
        )

    def _row_limitations(
        self, path: Path, extracted_from: Path | None, scan_limitations: str
    ) -> str:
        limitations = scan_limitations
        if (
            extracted_from is not None
            and extracted_from.suffix.lower() == ".etl"
            and self._file_size(path) > self.scan_settings.large_file_warning_bytes
        ):
            extra = "Decoded ETL output is large; scanned with streaming mode."
            limitations = f"{limitations} {extra}" if limitations else extra
        return limitations
