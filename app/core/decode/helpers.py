...
from app.core.util.config import ScanSettings
...
def find_procmon() -> Path | None:
    """Find ProcMon in portable tools first, then PATH."""
    return _find_tool(
        [app_root() / "tools" / "Procmon64.exe", app_root() / "tools" / "Procmon.exe"],
        ["Procmon64.exe", "Procmon.exe", "procmon64", "procmon"],
    )
...
def decode_optional_artifact(
    ...
    if artifact_family == "etl":
        return _decode_etl(path, decoded_root, artifact_id, scan_settings)
    if artifact_family == "procmon_pml":
        return _decode_pml(path, decoded_root, artifact_id, scan_settings)
    if artifact_family == "memory_dump":
        return _decode_dmp(path, scan_settings)
...
def _decode_etl(
    path: Path, decoded_root: Path, artifact_id: str, scan_settings: ScanSettings | None
) -> DecoderResult:
    tracerpt = find_tracerpt()
    if tracerpt is None:
        fallback = scan_binary_file(path, scan_settings)
        return DecoderResult(
            _replace_stats(
                fallback,
...
def _decode_pml(
    ...
    procmon = find_procmon()
    if procmon is not None:
        output_path = decoded_root / f"pml_{artifact_id}.csv"
        result = _run_tool([str(procmon), "/OpenLog", str(path), "/SaveAs", str(output_path)])
        if result.returncode == 0 and output_path.exists():
            return DecoderResult(
                _metadata_stats(
                    path,
                    decoder_used=procmon.name,
                    decoder_status="decoded-with-limitation",
                    limitations="ProcMon PML was converted to CSV for streaming review.",
                ),
                [output_path],
            )
    fallback = scan_binary_file(path, scan_settings)
    return DecoderResult(
        _replace_stats(
            fallback,
            decoder_used="procmon-pml",
            decoder_status="specialized-tool-unavailable",
            limitations=_append_limitation(
                "ProcMon PML binary was not fully decoded; binary string scan performed.",
...
def _decode_dmp(path: Path, scan_settings: ScanSettings | None) -> DecoderResult:
    fallback = scan_binary_file(path, scan_settings)
    return DecoderResult(
        _replace_stats(
            fallback,
            decoder_used="debugger-not-run",
            decoder_status="specialized-tool-unavailable",
            limitations=_append_limitation(
                "Memory dump was not debugger-decoded; binary string scan performed.",
                fallback.limitations,
            ),
        )
    )
...
def _append_limitation(primary: str, secondary: str) -> str:
    if secondary:
