...
def write_summary_reports(
    rows: list[ArtifactCoverageRow], hits: list[SearchHit], workspace: Path
) -> ReportOutputs:
...
def _render_text_report(
    ...
    lines = [
        ...
        f"Hits observed: {len(hits)}",
        f"Rules observed: {len({hit.rule_id for hit in hits})}",
        "",
        "Product detected",
        "----------------",
    ...
    lines.extend(["", "Raw hit appendix", "----------------"])
    for hit in hits:
        location = hit.line_number or hit.byte_offset
        lines.append(f"- {hit.rule_id} {hit.artifact_path}:{location} {hit.raw_line}")
    lines.extend(["", "Limitations", "-----------"])
    lines.extend(_limitations(rows))
    return "\n".join(lines).rstrip() + "\n"
...
""".strip()
    sections = [
        ...
        _html_section(
            ...
            f"Hits observed: {len(hits)}<br>"
            f"Rules observed: {len({hit.rule_id for hit in hits})}</p>",
        ),
    ...
    for severity in ("Critical", "High", "Medium", "Low", "Info"):
        sections.append(
            _html_section(
                f"{severity} findings",
                _html_hits([hit for hit in hits if hit.severity == severity]),
            )
        )
    sections.extend(
        [
            ...
            _html_section("Raw hit appendix", _html_hits(hits)),
            _html_section("Limitations", _html_list(_limitations(rows))),
            "</body></html>",
        ]
    )
    return "\n".join(sections)
...
def _text_finding_lines(hit: SearchHit) -> list[str]:
    return [
        ...
        f"  observed evidence: {hit.raw_line}",
        f"  context_before: {hit.context_before}",
        f"  context_after: {hit.context_after}",
...
def _limitations(rows: list[ArtifactCoverageRow]) -> list[str]:
    limitations = [f"{row.artifact_path}: {row.limitations}" for row in rows if row.limitations]
    limitations.append("KB candidates require independent validation.")
    limitations.append(
...
def _available_evidence(rows: list[ArtifactCoverageRow], hits: list[SearchHit]) -> set[str]:
    available = {row.artifact_family.lower() for row in rows}
    searchable_text = "\n".join(
