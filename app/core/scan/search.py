...
from app.core.util.config import ScanSettings, scan_settings
...
BINARY_SCANNABLE_FAMILIES = {"opaque_binary", "unknown", "etl", "procmon_pml", "memory_dump"}
_CONTEXT_LINES = 2
...
def scan_text_file(path: Path, settings: ScanSettings | None = None) -> ScanStats:
    """Stream a text-like file line by line and collect generic keyword hits."""
    active_settings = settings or scan_settings()
    ...
    for encoding in candidates:
        scan_stats = _scan_text_with_encoding(path, encoding, active_settings)
...
def _scan_text_with_encoding(path: Path, encoding: str, settings: ScanSettings) -> ScanStats:
    hits: list[SearchHit] = []
    ...
    try:
        with path.open("r", encoding=encoding, errors="strict", newline="") as text_file:
            for line_number, line in enumerate(text_file, start=1):
                ...
                line_text = _strip_line_ending(line)
                raw_line = _truncate_text(line_text, settings.max_context_chars)
                for pending_hit in list(pending_hits):
                    if len(pending_hit.after_lines) < _CONTEXT_LINES:
                        pending_hit.after_lines.append(raw_line)

                for line_hit in _line_hits(line_text):
                    pending_hits.append(
                        _PendingTextHit(
                            artifact_path=path,
    ...
    return ScanStats(
        ...
        encoding=encoding,
        limitations=_large_file_warning(bytes_scanned, settings),
        hits=hits,
    )


def scan_binary_file(path: Path, settings: ScanSettings | None = None) -> ScanStats:
    """Stream a binary file in fixed-size chunks and search extracted printable strings."""
    active_settings = settings or scan_settings()
    chunk_bytes = max(active_settings.binary_chunk_bytes, 1)
    overlap_bytes = max(active_settings.binary_overlap_bytes, 0)
    hits: list[SearchHit] = []
    seen_hit_keys: set[tuple[str, int, str]] = set()
    fragments_scanned = 0
    bytes_scanned = 0
    carry = b""
    ...
    try:
        with path.open("rb") as binary_file:
            while chunk := binary_file.read(chunk_bytes):
                ...
                fragments_scanned += _scan_ascii_fragments(
                    path, search_chunk, search_start, hits, seen_hit_keys
                )
                fragments_scanned += _scan_utf16le_fragments(
                    path, search_chunk, search_start, hits, seen_hit_keys
                )
                carry = search_chunk[-overlap_bytes:] if overlap_bytes else b""
    except OSError as exc:
        return ScanStats(
            review_mode="unreadable",
            lines_or_rows_scanned=0,
    ...
    warnings = []
    large_warning = _large_file_warning(bytes_scanned, active_settings)
    if large_warning:
        warnings.append(large_warning)
    if fragments_scanned == 0:
        warnings.append("No printable ASCII or UTF-16LE-like strings were found.")
...
def _large_file_warning(bytes_scanned: int, settings: ScanSettings) -> str:
    if bytes_scanned > settings.large_file_warning_bytes:
        return "File exceeds warning threshold; scanned using streaming/chunked mode."
    return ""
...
def _scan_ascii_fragments(
    ...
    hits: list[SearchHit],
    seen_hit_keys: set[tuple[str, int, str]],
) -> int:
    fragments = 0
    for match in _PRINTABLE_ASCII_RE.finditer(data):
        ...
        fragments += 1
        _append_binary_hits(path, fragment, byte_offset, "ascii-string", hits, seen_hit_keys)
    return fragments
        ...
        while index + 1 < len(data):
            ...
            if 0x20 <= char_byte <= 0x7E and null_byte == 0:
                ...
                if len(current) >= 4 and current_start is not None:
                    ...
                    fragments += 1
                    _append_binary_hits(
                        path, fragment, byte_offset, "utf-16le-string", hits, seen_hit_keys
                    )
                current = bytearray()
                current_start = None
            index += 2
...
def _append_binary_hits(
    ...
    for line_hit in _line_hits(fragment):
        hit_key = (line_hit["rule_id"], byte_offset, line_hit["matched_text"])
